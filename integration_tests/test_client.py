...
from integration_tests.constants import WEB_PROVIDER_URL, ETHEREUM_ADDRESS, ETHEREUM_PRIVATE_KEY
import goldlink.constants as Constants

ADDRESS_MANAGER_ADDRESS = Constants.CONTRACTS[Constants.ADDRESS_MANAGER][Constants.NETWORK_ID_ANVIL]
...
class TestClient():
    ...
    def test_reader_no_host_or_web3(self):
        with pytest.raises(Exception) as excinfo:
            Client()
        assert str(excinfo.value) == 'Web3 not passed in and cannot set web3 with no host or web3 provider.'

    def test_reader_web3(self):
        client = Client(
            network_id=Constants.NETWORK_ID_ANVIL,
